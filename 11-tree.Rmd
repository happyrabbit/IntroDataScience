...
$$\Sigma_{i:x_i\in R_1(j,s)}(y_i-\hat{y}*{R*{1}})^2+\Sigma_{i:x_i\in R_2(j,s)}(y_i-\hat{y}*{R*{2}})^2$$

where $\hat{y}{R_1}$ is the mean of all samples in $R_1$, $\hat{y}{R_2}$ is the mean of samples in $R_2$. It can be quick to optimize the equation above. Especially when $p$ is not too large. 
...
### Decision Tree
>>>>>>> 6932c1929a687fa11025a1158c401513992b965d
