...
1. Explanatory variables ($\mathbf{x_{.j}}$) and random error ($\symbf{\epsilon}$) are independent:  $cov(\mathbf{x_{.j},\epsilon})=0$ for $\forall j=j\in1...p$. 

2. The expected value of random error is zero: $E(\symbf{\epsilon|X})=0$

3. Random errors are independent with each other, and the variance of random error is consistent: $Var(\symbf{\epsilon})=\sigma^{2}I$, where $\sigma$ is positive and $I$ is a $n \times n$ identical matrix.
