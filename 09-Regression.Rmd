...
PLS is from Herman Wold's Nonlinear Iterative Partial Least Squares (NIPALS) algorithm [@wold1973;@wold1982] \index{Nonlinear Iterative Partial Least Squares (NIPALS)}. This algorithm was first used to linearize non-linear variables. Later NIPALS was applied to regression problems, which was then called PLS \index{PLS}. In general, it will iteratively find the relationship between highly correlated explanatory variables and the response variable. We will not discuss the PLS algorithm's detail, and the reference mentioned above provides a more detailed description of the algorithm. 
