...
Let's look at a simple one-layer neural network (figure \@ref(fig:onelayernn)). First only consider one sample, an input sample vector $x$ with 3 features ($x_1, x_2, x_3$) and an output value $y$. The neural network has one hidden layer with four neurons in the layer and output later to produce a prediction $\hat{y}$.
...
Each inactivated neuron on the first layer is a linear transformation of the input vector $x$. For example, $z^{[1]}_1 = w^{[1]T}_1x^{(i)} + b_1^{[1]}$ is the first inactivated neuron for layer one. We use `[]` on top right to indicate a layer and the bottom-right subscript to indicate a neuron or feature. Here $w^{[1]}$ and $b_1^{[1]}$ are the weight and bias parameters for layer 1. Then use a sigmoid function $\sigma(\cdot)$ to activate the neuron $z^{[1]}_1$ to get $a^{[1]}_1$. 
...
If you fully write out the process, it is the bottom right of figure \@ref(fig:onelayernn). You can vectorize each step and get the following representation:
...
This is the process of forward propagation for one sample. You need to do the same calculation for all  $m$ samples. 
...
You can further use matrix representation as following:
