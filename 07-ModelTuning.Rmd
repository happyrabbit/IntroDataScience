...
Bootstrap is a powerful statistical tool (a little magic too). It can be used to analyze the uncertainty of parameter estimates [@bootstrap1986] quantitatively. For example, estimate the standard deviation of linear regression coefficients. The power of this method is that it can be easily applied to each model (to put it bluntly, there is a random sampling and fitting process of putting back the data repeatedly), and some models can hardly obtain the standard deviation by using the traditional statistical inference method And confidence interval. 