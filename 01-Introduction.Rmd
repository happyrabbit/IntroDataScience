...
Prediction focuses on predicting and understanding the role each variable plays is not a concern. Many black-box models, such as ensemble methods and deep learning, are often used to make a prediction. Examples of problems are image recognition, machine translation, and recommendation.

The focus of modeling is to explain if you want to understand the drivers of an outcome of interest or relationship among variables. Explainability is critical when you need to find a way to intervene to achieve the desired result. For example, product managers often need to do feature prioritization where they want to order a list of features by user preference. A standard method for this problem is choice modeling, and you need to explain the drivers behind users' decisions on using/purchasing a product. 
