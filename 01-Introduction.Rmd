...
=======
For outsiders, data science is whatever magic that can get useful information out of data. Everyone should have heard about big data. Data science trainees now need the skills to cope with such big data sets. What are those skills? You may hear about: Hadoop, a system using Map/Reduce to process large data sets distributed across a cluster of computers, or hear about Spark, a system built atop Hadoop for speeding up the same by loading massive datasets into shared memory (RAM) across clusters with an additional suite of machine learning functions for big data. The new skills are for dealing with organizational artifacts of large data sets beyond a single computer’s memory or hard disk and large-scale cluster computing. These new skills do not necessarily lead to a better solution to the real problem. 
...
>>>>>>> 37ca0c40c5e389495872489975c09611331391d1

To declutter, let's start with a brief history of data science. If you search on the Google Trends, which shows search keyword information over time, the term "data science" back a little further than 2004. The way media describes it, you may feel that machine learning algorithms are new, and there was never "big" data before Google. That is not true. There are new and exciting developments in data science. But many of the techniques we are using are based on decades of work by statisticians, computer scientists, mathematicians, and scientists of many other fields.
...
In 1984, Breiman introduced the classification and regression tree (CART) \index{classification and regression tree} \index{CART}, one of the oldest and most utilized classification and regression techniques [@Breiman1984].

After that, Ross Quinlan came up with more tree algorithms such as ID3, C4.5, and C5.0. In the 1990s, ensemble techniques (methods that combine many models' predictions) began to appear. Bagging is a general approach that uses bootstrapping in conjunction with regression or classification model to construct an ensemble. Based on the ensemble idea, Breiman came up with the random forest \index{random forest} model in 2001 [@Breiman2001]. In the same year, Leo Breiman published a paper “Statistical Modeling: The Two Cultures” [@Breiman2001TwoCulture] where he pointed out two cultures in the use of statistical modeling to get information from data:
...
Tukey's 1962 list is surprisingly modern. Let's inspect those points in today's context. People usually develop theories way before they find potential applications. In the past 50 years, statisticians, mathematicians, and computer scientists have laid the theoretical groundwork for constructing "data science" today. The development of computers enables us to apply the algorithmic models (which can be very computationally expensive) and deliver results in a friendly and intuitive way. The striking transition to the internet and the internet of things generates vast amounts of commercial data. Industries have also sensed the value of exploiting that data. Data science seems sure to be a significant preoccupation of commercial life in the coming decades. All the four forces John Tukey identified exist today and have been driving data science.
>>>>>>> 37ca0c40c5e389495872489975c09611331391d1
...
So, the answer to the question "what is data science?" depends on who you are talking to.  Data science has three main skill tracks: engineering, analysis, and modeling (and yes, the order matters!).
...
<<<<<<< HEAD
Automated data collection is a common task that includes parsing the logs (depending on the stage of the company and the type of industry you are in), web scraping, API queries, and interrogating data streams. Data management includes constructing data schema to support analytical and modeling needs, and ensuring data is correct, standardized, and documented.
=======
Automated data collection is a common task that includes parsing the logs (depending on the stage of the company and the type of industry you are in), web scraping, API queries, and interrogating data streams. Determine and construct data schema to support analytics and modeling needs. Use tools, processes, guidelines to ensure data is correct, standardized, and documented.
>>>>>>> 37ca0c40c5e389495872489975c09611331391d1
...
Often, the data quality is more important than the quantity, but you can not completely overlook quantity.  
Suppose you can guarantee data quality, even then the more data, the better. If we have a specific and reasonable question with sound and relevant data, then congratulations, we can start playing data science!
...
![Different roles in data science and the skill requirements](../course2020/slides/01Introduction/images/dsrole.png)
